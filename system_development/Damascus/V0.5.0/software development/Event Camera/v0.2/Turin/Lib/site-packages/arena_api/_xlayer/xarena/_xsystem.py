...
import os
import sys
from ctypes import byref, create_string_buffer, cast, py_object
...
from arena_api._xlayer.xarena.arenac_types import (
    acCallback, acDevice, acDeviceDisconnectCallbackFunction, acNodeMap,
    acSystem, bool8_t, size_t, uint32_t, uint64_t, char_ptr, void_ptr)
...
class _xSystem:
    ...
    def xSystemRegisterDeviceDisconnectCallback(self, py_callback_function, device,
                                                args_and_kwargs):

        handle = acCallback(None)
        ac_device = acDevice(device)
        c_callback_function = acDeviceDisconnectCallbackFunction(
        ...
        # configured
        #
        # harenac.acSystemRegisterDeviceDisconnectCallback.argtypes = [
        #    acSystem,
        #    acDevice,
        #    POINTER(acCallback),
        #    acDeviceDisconnectCallbackFunction callbackFn]

        harenac.acSystemRegisterDeviceDisconnectCallback(
            self.__acsystem,
            ac_device,
            byref(handle),
            c_callback_function
    ...
    def xSystemDeregisterDeviceDisconnectCallback(self, handle):
        ...
        # configured
        #
        # harenac.acSystemDeregisterDeviceDisconnectCallback.argtypes = [
        #   acSystem,
        #   acCallback]
        harenac.acSystemDeregisterDeviceDisconnectCallback(
            self.__acsystem,
            handle
        )

    def xSystemAddUnicastDiscoveryDevice(self, ip):

        ip_p = char_ptr(ip.encode())

        # AC_ERROR acSystemAddUnicastDiscoveryDevice(
        #   acSystem hSystem,
        #   char* pUnicastDeviceIP)
        harenac.acSystemAddUnicastDiscoveryDevice(
            self.__acsystem,
            ip_p)

    def xSystemRemoveUnicastDiscoveryDevice(self, ip):

        if ip is not None:
            ip = ip.encode()

        ip_p = cast(ip, void_ptr)

        # AC_ERROR acSystemRemoveUnicastDiscoveryDevice(
        #   acSystem hSystem,
        #   char* pUnicastDeviceIP)
        harenac.acSystemRemoveUnicastDiscoveryDevice(
            self.__acsystem,
            ip_p)