...
from arena_api._xlayer.xarena.arenac_types import (
    ...
    acCallback, acCallbackFunction, acChunkdata, acDevice,
    acDeviceDisconnectCallbackFunction, acFeatureStream, acOverlayInfo,
    acImageCallbackFunction, acNode, acNodeMap, acSocket, acSystem, bool8_t, char_ptr,
    double, unsigned_short, int64_t, size_t, uint8_t, uint32_t, uint64_t, void_ptr)
...
class ArenaCConfigurator:
    ...
    def _configure_global(self):
        ...
        # AC_ERROR acGetVersion(
        #   char* pVersionBuf,
        #   size_t * pBufLen)
        self.handle.acGetVersion.argtypes = [
            char_ptr,
            POINTER(size_t)]
    ...
    def _configure_system(self):
        ...
        c_funcs_list = [
            ...
            self.handle.acSystemRegisterDeviceDisconnectCallback,
            self.handle.acSystemDeregisterDeviceDisconnectCallback,
            self.handle.acSystemDeregisterAllDeviceDisconnectCallbacks
        ]
        for func_ptr in c_funcs_list:
            func_ptr.errcheck = self.raise_if_error
        ...
        # AC_ERROR acSystemRegisterDeviceDisconnectCallback(
        #   acSystem hSystem,
        #   acDevice hDevice,
        #   acCallback* phDeviceDisconnectCallback,
        #   acDeviceDisconnectCallbackFunction callbackFn)
        self.handle.acSystemRegisterDeviceDisconnectCallback.argtypes = [
            acSystem,
            acDevice,
            POINTER(acCallback),
        ...
        # AC_ERROR acSystemDeregisterDeviceDisconnectCallback(
        #   acSystem hSystem,
        #   acCallback hDeviceDisconnectCallback)
        self.handle.acSystemDeregisterDeviceDisconnectCallback.argtypes = [
            acSystem,
            acCallback
        ]

        # AC_ERROR acSystemDeregisterAllDeviceDisconnectCallbacks(
        #   acSystem hSystem)
        self.handle.acSystemDeregisterAllDeviceDisconnectCallbacks.argtypes = [
            acSystem]

    def _configure_socket(self):

        c_funcs_list = [
            self.handle.acSocketCreate,
            self.handle.acSocketDestroy,
            self.handle.acSocketOpenSender,
            self.handle.acSocketCloseSender,
            self.handle.acSocketAddDestination,
            self.handle.acSocketSendMessage,
            self.handle.acSocketSendImage,
            self.handle.acSocketOpenListener,
            self.handle.acSocketCloseListener,
            self.handle.acSocketReceiveMessage,
            self.handle.acSocketReceiveImage,
        ]
        for func_ptr in c_funcs_list:
            func_ptr.errcheck = self.raise_if_error
        ...
        #   acSocekt hSocket);
        self.handle.acSocketCreate.argtypes = [
            acSocket]
        
        # AC_ERROR acSocketDestroy(
        #   acSocket hSocket)
        self.handle.acSocketDestroy.argtypes = [
            acSocket]
        
        # AC_ERROR acSocketOpenSender(
        #   acSocket hSocket)
        self.handle.acSocketOpenSender.argtypes = [
            acSocket]
        
        # AC_ERROR acSocketCloseSender(
        #   acSocket hSocket)
        self.handle.acSocketCloseSender.argtypes = [
            acSocket]
        
        # AC_ERROR acSocketAddDestination(
        #   acSocket hSocket,
        #   unsigned short port)
        self.handle.acSocketAddDestination.argtypes = [
            acSocket,
            unsigned_short]
        ...
        #   acSocket hSocket,
        #   const char* pMessage)
        self.handle.acSocketSendMessage.argtypes = [
            acSocket,
            char_ptr]
        ...
        #   acSocket hSocket,
        #   acBuffer hBuffer)
        self.handle.acSocketSendImage.argtypes = [
            acSocket,
            acBuffer]
        ...
        #   acSocket hSocket,
        #   unsigned short port)
        self.handle.acSocketOpenListener.argtypes = [
            acSocket,
            unsigned_short]
    
        # AC_ERROR acSocketCloseListener(
        #   acSocket hSocket)
        self.handle.acSocketCloseListener.argtypes = [
            acSocket]
        ...
        #   char* pMessageBuf,
        #   size_t* pBufLen)
        self.handle.acSocketReceiveMessage.argtypes = [
            acSocket,
            char_ptr,
            POINTER(size_t)]
        
        # AC_ERROR acSocketReceiveImage(
        #   acSocket hSocket,
        #   acBuffer* phBuffer)
        self.handle.acSocketReceiveImage.argtypes = [
            acSocket,
            POINTER(acBuffer)]
    ...
    def _configure_imagefactory(self):
        ...
        c_funcs_list = [
            ...
            self.handle.acImageFactoryCreateEmpty,
            self.handle.acImageFactoryShallow,
            ...
            self.handle.acImageFactorySelectBits,
            self.handle.acImageFactorySelectBitsAndScale,
            self.handle.acImageFactoryProcessSoftwareLUT,
            self.handle.acImageFactoryDeinterleaveChannels,
            self.handle.acImageFactoryDeinterleaveChannelsShallow,
            self.handle.acImageFactoryDeinterleaveChannelsLen,
            self.handle.acImageFactorySplitChannels,
            self.handle.acImageFactoryOverlay,
            self.handle.acImageFactoryCreateOverlayInfo,
            self.handle.acImageFactoryDestroyOverlayInfo
        ]
        for func_ptr in c_funcs_list:
            func_ptr.errcheck = self.raise_if_error
        ...
        #   uint64_t pixelFormat,
        #   acBuffer* phDst)
        self.handle.acImageFactoryCreateEmpty.argtypes = [
            size_t,
            size_t,
            size_t,
            uint64_t,
        ...
        #   uint64_t pixelFormat,
        #   acBuffer* phDst)
        self.handle.acImageFactoryShallow.argtypes = [
            POINTER(uint8_t),
            size_t,
            size_t,
            size_t,
        ...
        #   int offset,
        #   acBuffer* phDst)
        self.handle.acImageFactorySelectBits.argtypes = [
            acBuffer,
            size_t,
            int64_t,
            POINTER(acBuffer)]
        ...
        #   double offset, 
        #   acBuffer* phDst)
        self.handle.acImageFactorySelectBitsAndScale.argtypes = [
            acBuffer,
            size_t,
            double,
            POINTER(acBuffer)]
        ...
        #   size_t len,
        #   acBuffer* phDst)
        self.handle.acImageFactoryProcessSoftwareLUT.argtypes = [
            acBuffer,
            POINTER(uint8_t),
            size_t,
            POINTER(acBuffer)]
        ...
        #   acBuffer hSrc, 
        #   acBuffer* phDst)
        self.handle.acImageFactoryDeinterleaveChannels.argtypes = [
            acBuffer,
            POINTER(acBuffer)]
        ...
        #   size_t len, 
        #   acBuffer* phDst)
        self.handle.acImageFactoryDeinterleaveChannelsShallow.argtypes = [
            acBuffer,
            POINTER(uint8_t),
            size_t,
            POINTER(acBuffer)]
        ...
        #   acBuffer hSrc,
        #   size_t* len)
        self.handle.acImageFactoryDeinterleaveChannelsLen.argtypes = [
            acBuffer,
            POINTER(size_t)]
        
        # AC_ERROR acImageFactorySplitChannels(
        #   acBuffer hSrc,
        #   acBuffer* phDst[],
        #   size_t* numBuffers)
        self.handle.acImageFactorySplitChannels.argtypes = [
            acBuffer,
            POINTER(acBuffer),
            POINTER(size_t)]
        
        # AC_ERROR acImageFactoryOverlay(
        #   acBuffer hDepthSrc,
        #   acBuffer hRgbSrc,
        #   acOverlayInfo hOverlayInfo,
        #   acBuffer* phDst)
        self.handle.acImageFactoryOverlay.argtypes = [
            acBuffer,
            acBuffer,
            acOverlayInfo,
            POINTER(acBuffer)]
        
        # AC_ERROR acImageFactoryCreateOverlayInfo(
        #   acDevice hChroma, 
        #   acDevice hHelios, 
        #   acOverlayInfo* phOverlayInfo)
        self.handle.acImageFactoryCreateOverlayInfo.argtypes = [
            acDevice,
            acDevice,
            POINTER(acOverlayInfo)]
        
        # AC_ERROR acImageFactoryDestroyOverlayInfo(
        #   acOverlayInfo hOverlayInfo)
        self.handle.acImageFactoryDestroyOverlayInfo.argtypes = [
            acOverlayInfo]
        
    def _configure_overlay_info(self):

        c_funcs_list = [
            self.handle.acOverlayInfoGetScale,
            self.handle.acOverlayInfoGetCoordinateOffsetA,
            self.handle.acOverlayInfoGetCoordinateOffsetB,
            self.handle.acOverlayInfoGetCoordinateOffsetC
        ]
        for func_ptr in c_funcs_list:
            func_ptr.errcheck = self.raise_if_error

        # AC_ERROR acOverlayInfoGetScale(
        #   acOverlayInfo hOverlayInfo,
        #   double* pScale) 
        self.handle.acOverlayInfoGetScale.argtypes = [
            acOverlayInfo,
            POINTER(double)]

        # AC_ERROR AC_API acOverlayInfoGetCoordinateOffsetA(
        #   acOverlayInfo hOverlayInfo,
        #   double* pCoordinateOffsetA)
        self.handle.acOverlayInfoGetCoordinateOffsetA.argtypes = [
            acOverlayInfo,
            POINTER(double)]
        
        # AC_ERROR AC_API acOverlayInfoGetCoordinateOffsetB(
        #   acOverlayInfo hOverlayInfo,
        #   double* pCoordinateOffsetB)
        self.handle.acOverlayInfoGetCoordinateOffsetB.argtypes = [
            acOverlayInfo,
            POINTER(double)]
        
        # AC_ERROR AC_API acOverlayInfoGetCoordinateOffsetC(
        #   acOverlayInfo hOverlayInfo,
        #   double* pCoordinateOffsetC)
        self.handle.acOverlayInfoGetCoordinateOffsetC.argtypes = [
            acOverlayInfo,
            POINTER(double)]
    ...
    def _configure_selector(self):
        ...
        # AC_ERROR acSystemAddUnicastDiscoveryDevice(
        #   acSystem hSystem,
        #   char* pUnicastDeviceIP)
        self.handle.acSystemAddUnicastDiscoveryDevice.argtypes = [
            acNode,
            char_ptr]

        # AC_ERROR acSystemRemoveUnicastDiscoveryDevice(
        #   acSystem hSystem,
        #   char* pUnicastDeviceIP)
        self.handle.acSystemRemoveUnicastDiscoveryDevice.argtypes = [
            acNode,
            void_ptr]
