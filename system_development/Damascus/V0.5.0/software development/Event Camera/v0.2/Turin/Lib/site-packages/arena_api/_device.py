...
from arena_api._xlayer.xarena.arenac_defaults import \
    WAIT_FOR_NEXT_LEADER_MILLISEC_DEFAULT as \
    _WAIT_FOR_NEXT_LEADER_MILLISEC_DEFAULT
...
class Device():
    ...
    **-------------------------------------------------------------------**
    ...
    in the output queue, otherwise, a ``TimeoutError`` will be raised.
    ...
    **-------------------------------------------------------------------**
    ...
    in the events output queue, otherwise, a ``TimeoutError`` will be raised.
    ...
    **-------------------------------------------------------------------**
    ...
    def start_stream(self, number_of_buffers=None):
        ...
        **Returns**:
        ...
        the stream. This includes the buffer\
        ...
        There are three ways to start and stop stream:\n
            - As a regular function call:
                With this way, the user has control over when to call\
            ...
            - As a regular function call but without calling a stop on the\
            stream:\n
    ...
    **-------------------------------------------------------------------**
    ...
    def get_buffer(self, number_of_buffers=1, timeout=None):
        ...
        **Returns**:\n
        ...
        It is a best practice to requeue buffers with \
        ``device.requeue_buffer()`` as soon as they are no longer needed.\
        If image data is needed for longer \
        (i.e. for processing), it is recommended to copy \
    ...
    def __check_requeue_buffer_list_input(self, buffers_list):
        ...
        if len(buffers_list) == 0:
            raise ValueError(
                'requeue_buffer argument cannot be an empty list')
    ...
    def requeue_buffer(self, buffers):
        ...
        **Returns**:\n
        ...
        longer (i.e. for processing), it is \
    ...
    def deinitialize_events(self):
        ...
        has been initialized with ``device.initialize_events()`` and after\
        all events have been processed with ``device.wait_on_event()``.\n
        ...
        **Returns**:\n
        ...
        infrastructure's initialization. It does the following:\n
    ...
    def __get_nodemap(self):
        ...
        nodemap = _nodemap.Nodemap(hxnodemap)

        nodemap._nodemap_type_name = 'device_nodemap'
        return nodemap
    ...
    device is created with ``system.create_device()`` and deinitialized\
    when the device is destroyed with ``system.destroy_device()``.\
    ...
    **device.tl_device_nodemap vs device.nodemap**:\n
    ...
    :warning:\n
    - Provides access to main node map ``device.nodemap``, which is not to be \
    ...
    **-------------------------------------------------------------------**
    ...
    def __get_tl_device_nodemap(self):
        ...
        nodemap._nodemap_type_name = 'device_tl_device_nodemap'
        return nodemap
    ...
    **-------------------------------------------------------------------**
    ...
    def __get_tl_stream_nodemap(self):
        ...
        nodemap._nodemap_type_name = 'device_tl_stream_nodemap'
        return nodemap
    ...
    **-------------------------------------------------------------------**
    ...
    def __get_tl_interface_nodemap(self):
        ...
        nodemap._nodemap_type_name = 'device_tl_interface_nodemap'
        return nodemap
    ...
    **-------------------------------------------------------------------**
    ...
    def __check_wait_for_next_leader_input_timeout(self, timeout_millisec):
        # non blocking call so timeout can be zero
        # input checks
        if timeout_millisec is None:
            timeout_millisec = _WAIT_FOR_NEXT_LEADER_MILLISEC_DEFAULT

        is_int = isinstance(timeout_millisec, int)
        is_inf = math.isinf(timeout_millisec)
        if not is_int and not is_inf:
            raise TypeError(f'expected int or math.inf instead of '
                            f'{type(timeout_millisec).__name__}')

        if timeout_millisec < 0:
            raise ValueError('timeout must be >= 0 or math.inf')

        if timeout_millisec is math.inf:
            timeout_millisec = _AC_INFINITE

        return timeout_millisec

    def wait_for_next_leader(self, timeout_millisec=None):
        """
        Will wait until the leader for the next image buffer
        has arrived. It must be called after the stream has started
        and before the stream has stopped.
        This function can be used to determine when the host has received the
        leader for the next image buffer. Note that if the time that the camera has
        finished the exposure for the next buffer is desired, it is
        recommended to use the GenICam ExposureEnd Event instead.

        **Args**:
        - timeout_millisec: can be
            - A positive ``int`` value that represents
            the maximum time, in millisec, to wait for next buffer.
            The value zero will return a buffer if there is a
            ready buffer in the output queue, otherwise, a
            ``TimeoutError`` will be raised.

        **Raises**:
        - ``TimeoutError``:
            - ``ArenaSDK`` is not able to get next leader before
            the timeout expiration
        ...
        **Returns**:
        ...
        """
        timeout_millisec = self.__check_wait_for_next_leader_input_timeout(timeout_millisec)

        self._xdev.xDeviceWaitForNextLeader(timeout_millisec)

    def reset_wait_for_next_leader(self):
        """
        Clears any pending flag for a received
        leader event.

        **Returns**:
        - ``None``.
        """
        self._xdev.xDeviceResetWaitForNextLeader()

    def is_connected(self):
        """
        Returns true if a device has been opened and
        maintains a valid communication socket. The device is opened when
        system.create_device() is called. If the connection to the device
        is lost this will return false.

        **Returns**:
        - ``True`` or `False``
        """
        return self._xdev.xDeviceIsConnected()
