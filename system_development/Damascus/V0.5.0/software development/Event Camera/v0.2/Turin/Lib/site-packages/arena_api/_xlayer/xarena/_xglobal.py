...
class _xGlobal():
    ...
    @staticmethod
    def xGetArenaCbuildVersion():
        version_p = create_string_buffer(XARENA_STR_BUFFER_SIZE_DEFAULT)
        buf_len = size_t(XARENA_STR_BUFFER_SIZE_DEFAULT)

        # AC_ERROR acGetVersion(
        #   char* pVersionBuf,
        #   size_t * pBufLen)
        harenac.acGetVersion(
            version_p,
            byref(buf_len))
    ...
    @staticmethod
    def xGetGetBitsPerPixel(pixel_format):
        pixel_format = uint64_t(pixel_format)
        bits_per_pixel = size_t(XARENA_STR_BUFFER_SIZE_DEFAULT)

        # AC_ERROR acGetBitsPerPixel(
        # uint64_t pixelFormat,
        # size_t* pBitsPerPixel)
        harenac.acGetBitsPerPixel(
            pixel_format,
            byref(bits_per_pixel))

        return bits_per_pixel.value
