...
class _xBuffer():
    ...
    def xBufferIsCompressedImage(self):

        is_compressedimage = bool8_t(False)
        # AC_ERROR acBufferIsCompressedImage(
        #   acBuffer hBuffer,
        #   bool8_t * pIsCompressedImage)
        harenac.acBufferIsCompressedImage(
            self.hxbuffer,
            byref(is_compressedimage))

        return is_compressedimage.value
    ...
    def xCompressedImageGetPixelFormat(self):

        pixel_format = uint64_t(0)
        # AC_ERROR acCompressedImageGetPixelFormat(
        #   acBuffer hBuffer,
        #   uint64_t* pPixelFormat)
        harenac.acCompressedImageGetPixelFormat(
            self.hxbuffer,
            byref(pixel_format))

        return pixel_format.value

    def xCompressedImageGetTimestamp(self):

        timestamp = uint64_t(0)
        # AC_ERROR acCompressedImageGetTimestamp(
        #   acBuffer hBuffer,
        #   uint64_t* pTimestamp)
        harenac.acCompressedImageGetTimestamp(
            self.hxbuffer,
            byref(timestamp))

        return timestamp.value

    def xCompressedImageGetTimestampNs(self):

        timestampns = uint64_t(0)
        # AC_ERROR acCompressedImageGetTimestampNs(
        #   acBuffer hBuffer,
        #   uint64_t* pTimestampNs)
        harenac.acCompressedImageGetTimestampNs(
            self.hxbuffer,
            byref(timestampns))

        return timestampns.value

    def xCompressedImageGetData(self):

        pdata = POINTER(uint8_t)(uint8_t(0))
        # AC_ERROR acCompressedImageGetData(
        #   acBuffer hBuffer,
        #   uint8_t** ppData)
        harenac.acCompressedImageGetData(self.hxbuffer, byref(pdata))
