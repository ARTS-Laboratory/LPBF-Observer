...
class _xDevice():
    ...
    def xDeviceWaitForNextLeader(self, timeout_millisec):

        timeout_millisec = uint64_t(timeout_millisec)
        # AC_ERROR acDeviceWaitForNextLeader(
        #   acDevice hDevice,
        #   int64_t timeout)
        harenac.acDeviceWaitForNextLeader(self.hxdevice, timeout_millisec)

    def xDeviceResetWaitForNextLeader(self):

        # AC_ERROR acDeviceResetWaitForNextLeader(
        #   acDevice hDevice)
        harenac.acDeviceResetWaitForNextLeader(self.hxdevice)

    def xDeviceIsConnected(self):

        is_connected = bool8_t(False)
        # AC_ERROR acDeviceIsConnected(
        #   acDevice hDevice,
        #   bool8_t* pIsConnected)
        harenac.acDeviceIsConnected(self.hxdevice,  byref(is_connected))
