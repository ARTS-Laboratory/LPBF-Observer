...
This module provides callback registration and management
for devices, nodes, and systems in Arena API.

- Use :class:`arena_api.callback._Callback` via the singleton
  instance :data:`arena_api.callback.callback`.
- Use :class:`arena_api.callback._CallbackFunction` via the
  instance :data:`arena_api.callback.callback_function` to
  decorate Python functions for events.
...
from arena_api import _device as _device
...
from arena_api.system import _System as _System
...
class _Callback:
    ...
    Every other import after that returns the same instance.
    - Deregister handles before destroying it or memory will leak.
    - Import the instance ``callback`` not the class itself as this class \
    ...
    **-------------------------------------------------------------------**
    ...
    def __init__(self):
        ...
        self._system_callback = _SystemCallback()
    ...
    def register(self, obj, callback_function, *args, **kwargs):
        ...
        Registers a Python function as a callback function to an ``arena_api``\
         object. The supported objects are:
            ...
            - ``Node``
                - ``on_update`` event
            - ``System``
                - ``on_device_disconnected`` event

        **Args**:
            obj :
                - Must be an instance of ``_Device``, ``_Node`` or ``_System``.
            callback_function :
                ...
                For decorator  ``@callback.system.on_device_disconnected``, the \
                signature is ``callback_func(device)``\n

                Buffer, node, and device will be passed to the callback function when \
                the event happens. The user should pass the optional arguments\
                when registering the callback function on the object so \
                the user will receive them at the callback later \n
                Example:\n
                    ...
                    >>> # now every time the node gets invalidated by modification or by parents and child nodes
        ...
        **-------------------------------------------------------------------**
        ...
        if not decorated or callback_function._decorator not in self._supported_decorators:
            ...
            raise ValueError(f'\'{callback_function}\' must be decorated to be '
        ...
        if isinstance(obj, _Device):
            ...
            registry_entry = self._system_callback.register(obj,
                                                            callback_function,
                                                            *args,
                                                            **kwargs)
        else:
    ...
    def deregister(self, callback_handle):
        ...
        returned from when the calls ``callback.register()``. \n
    ...
    def _deregister_handle(self, callback_handle):
        ...
        if isinstance(obj, _Device):
            ...
            self._system_callback.deregister(obj, callback_handle)
        else:
            raise ValueError(f'internal error : {obj} is not a supported type')
...
class _SystemCallback:

    @staticmethod
    def register(system, callback_function, *args, **kwargs):
        args_and_kwargs = [args, kwargs]
        registry_entry = {
            'obj': system,
            'callback_function': callback_function,
        ...
        # xlayer call
        device = kwargs['watched_device']

        if device is None:
            raise Exception('watched_device is None')

        device_handle = device._xdev.hxdevice.value

        callback_handle, to_add_to_registry_entry = system._xsystem.xSystemRegisterDeviceDisconnectCallback(callback_function,
                                                                                                            device_handle, args_and_kwargs)
        registry_entry.update(to_add_to_registry_entry)
        return {callback_handle: registry_entry}

    @staticmethod
    def deregister(system, callback_handle):
        try:
            system._xsystem.xSystemDeregisterDeviceDisconnectCallback(
                callback_handle)
        except OSError as os_error:
            if 'exception: access violation reading' in str(os_error):
                raise ValueError('handle is registered for a destroyed device. Please '
...
class _CallbackFunction:
    ...
    with ``callback_register()``. The available decorators are:\n
    - ``Device`` callbacks:
        - ``@callback_function.device.on_buffer`` :
            Decorates a function to be used for a callback when a buffer
            arrives at the device. A function decorated with this must have
    ...
    - ``Node`` callbacks:
        - ``@callback_function.node.on_update`` :
            ...
            decorated with this must have the signature
            ``my_callback(node , *args, **kwargs)``, where node
            is a mandatory parameter.
    :Warning:
    - Import the instance ``callback_function`` not the class itself.
    ...
    def __init__(self):
        ...
        self.system = _SystemCallbackFunctionDecorator()
        self.supported_decorators = self.device.supported_decorators + \
            self.node.supported_decorators + self.system.supported_decorators
...
class _SystemCallbackFunctionDecorator:
    supported_decorators = ('callback_function.system.on_device_disconnected',)
    ...
    @staticmethod
    def on_device_disconnected(callback_function):
        callback_function._decorator = f'callback_function.system.on_device_disconnected'

        @wraps(callback_function)
        def wrapper_func(device):
            py_device = _Device(device)

            return callback_function(py_device)
...
#: Singleton instance of :class:`_Callback` for registering/deregistering callbacks.
callback = _Callback()

#: Singleton instance of :class:`_CallbackFunction` providing decorators.
callback_function = _CallbackFunction()

__all__ = ["callback", "callback_function"]
