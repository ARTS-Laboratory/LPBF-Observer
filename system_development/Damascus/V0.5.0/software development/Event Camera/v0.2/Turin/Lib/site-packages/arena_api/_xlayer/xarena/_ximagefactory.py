...
class _xImagefactory():
    ...
    @staticmethod
    def xImageFactoryOverlay(hdepth_src_buffer, hrgb_src_buffer, hoverlay_info):

        depth_src_buffer = acBuffer(hdepth_src_buffer)
        rgb_src_buffer = acBuffer(hrgb_src_buffer)
        overlay_info = acOverlayInfo(hoverlay_info)
        dst_buffer = acBuffer(None)
    ...
    @staticmethod
    def xImageFactoryCreateOverlayInfo(hchroma, hhelios):

        chroma = acDevice(hchroma)
        helios = acDevice(hhelios)
        dst_overlay_info = acOverlayInfo(None)
        
        # AC_ERROR acImageFactoryCreateOverlayInfo(
        #   acDevice hChroma,
        #   acDevice hHelios,
        #   acOverlayInfo* phOverlayInfo);
        harenac.acImageFactoryCreateOverlayInfo(
            chroma,
            helios,
            byref(dst_overlay_info)
        )
        
        return dst_overlay_info.value

    @staticmethod
    def xImageFactoryDestroyOverlayInfo(hoverlay_info):

        overlay_info = acOverlayInfo(hoverlay_info._xoverlay_info.hxoverlay_info.value)
        
        # AC_ERROR acImageFactoryDestroyOverlayInfo(
        #   acOverlayInfo hOverlayInfo);
        harenac.acImageFactoryDestroyOverlayInfo(
            overlay_info
        )