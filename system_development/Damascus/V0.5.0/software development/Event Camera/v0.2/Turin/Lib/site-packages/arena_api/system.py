...
class _System():
    ...
    other import after that returns the same instance.
    ...
    **-------------------------------------------------------------------**
    ...
    def __open(self):
        if not self._xsystem:
            hxsystem = _xGlobal.xOpenSystem()
            self._xsystem = _xSystem(hxsystem)
    ...
    def __close(self):
        if self._xsystem:
            ...
            _xGlobal.xCloseSystem(self._xsystem.hxsystem)
    ...
    **-------------------------------------------------------------------**
    ...
    def __get_interface_infos(self):
        num_of_interfaces = self._xsystem.xSystemGetNumInterfaces()
        all_interfaces_info = []
        for index in range(num_of_interfaces):
            interface_info = {}
            interface_info['ip'] = self._xsystem.xSystemGetInterfaceIpAddressStr(
                index)
            interface_info['subnetmask'] = self._xsystem.xSystemGetInterfaceSubnetMaskStr(
                index)
            interface_info['mac'] = self._xsystem.xSystemGetInterfaceMacAddressStr(
                index)
    ...
    -----------------------------------------------------------------------
    **ip**: A string value that represents the IP address, in
    ...
    -----------------------------------------------------------------------
    **subnetmask**: A string value that represents the subnet mask, in
    dot-decimal notation, of the interface on the host.
    An interface has its IP address, subnet mask, and MAC address checked
    through the Transport Layer Interface node map:
    ``GevInterfaceIPAddress``, ``GevInterfaceSubnetMask``,
    ...
    **-------------------------------------------------------------------**
    ...
    def __get_device_infos(self):
        ...
        device_info_has_changed = self._xsystem.xSystemUpdateDevicesHasChanged(
            self.DEVICE_INFOS_TIMEOUT_MILLISEC)

        num_of_devices = self._xsystem.xSystemGetNumDevices()
        device_infos = []
        for index in range(num_of_devices):
            device_info = {}

            device_info['model'] = self._xsystem.xSystemGetDeviceModel(index)
            device_info['vendor'] = self._xsystem.xSystemGetDeviceVendor(
                index)
            device_info['serial'] = self._xsystem.xSystemGetDeviceSerial(
                index)
            device_info['ip'] = self._xsystem.xSystemGetDeviceIpAddressStr(
                index)
            device_info['subnetmask'] = self._xsystem.xSystemGetDeviceSubnetMaskStr(
                index)
            device_info['defaultgateway'] = self._xsystem.xSystemGetDeviceDefaultGatewayStr(
                index)
            device_info['mac'] = self._xsystem.xSystemGetDeviceMacAddressStr(
                index)
            device_info['name'] = self._xsystem.xSystemGetDeviceUserDefinedName(
                index)
            device_info['version'] = self._xsystem.xSystemGetDeviceVersion(
                index)
            device_info['dhcp'] = self._xsystem.xSystemIsDeviceDHCPConfigurationEnabled(
                index)
            device_info['persistentip'] = self._xsystem.xSystemIsDevicePersistentIpConfigurationEnabled(
                index)
            device_info['lla'] = self._xsystem.xSystemIsDeviceLLAConfigurationEnabled(
                index)
    ...
    ``persistentip``, ``lla``, and ``version``. User changes  the
    values of the dictionary will not reflect on the device.\n
    -----------------------------------------------------------------------
    ...
    **serial**: A string value that represents the serial number of the
    ...
    -----------------------------------------------------------------------
    **ip**: A string value that represents the discovered device
    ...
    -----------------------------------------------------------------------
    **subnetmask**: a string value that represents the discovered device
    ...
    **dhcp**: A bool value that represents whether DHCP is enabled
    ...
    -----------------------------------------------------------------------
    **persistentip**: A bool value that represents whether
    ...
    **-------------------------------------------------------------------**
    ...
    def create_device(self, device_infos=None):
        ...
        **Args**:
            device_infos : can be\n
                - A list of device info dicts which is obtained from\
    ...
    def __validate_device_info_before_create_device(self, device_info):
        ...
        # list has an element that is not dict
            ...
            raise TypeError(f'Expected a list of dicts instead of a list '
                            f'that has a {type(device_info).__name__} '
                            f'element')
    ...
    def __create_new_device(self, device_info):
        ...
        hxdevice = self._xsystem.xSystemCreateDevice(device_index)
    ...
    def select_device(self, devices=None):
        """
        Selects one ``arena_api._device.Device`` instance\
        from devices list argument.
        ...
        **Returns**:
        ...
        The returned device is ready to stream images,
        send events, and read or customize features.

        :warning:
        - The ``arena_api._device.Device`` instance(s) in the devices list\
        have to be created by calling ``system.create_device()``.
        - Devices must be destroyed.
        ...
        **-------------------------------------------------------------------**
        ...
        if isinstance(devices, list):
            if len(devices) == 0:
                raise ValueError('input cannot be an empty list')
            elif not all(isinstance(d, _Device) for d in devices):
                raise TypeError(f'Expected list of arena_api._device.Device '
                            f'instead of {type(devices).__name__}')

        elif devices is None:
            devices = list(self.__created_devices.values())
        ...
        if num_devices == 1:
            print(f'{TAB1}Only one device detected: ', devices[0])
            print(f'{TAB2}Automatically selecting this device.')
            return devices[0]
        
        print(f'{TAB1}Select device:')
        for num, device in enumerate(devices):
            print(f'{TAB2}{num+1}. {device}')

        while True:
            try:
                selection = int(input(TAB2 + 'Make selection (1-' + str(num_devices) + '): '))
            except ValueError:
                print(f'{TAB2}Invalid device selected, please select a device in range.')
                continue

            if 0 < selection and selection <= num_devices:
    ...
    def destroy_device(self, device=None):
        ...
        the system will be called it when the module unloads.\n
        ...
        **Args**:
            device: can be\n
                ...
                reference cannot be used after calling this function\n
        **Raises**:
            - ``ValueError`` :
    ...
    """
        ...
        for device in devices_as_list:
            ...
            self._xsystem.xSystemDestroyDevice(device._xdev.hxdevice.value)
    ...
    def force_ip(self, device_info):
        ...
        needed to force the new ip are: MAC address of the device, ip, subnet mask,
        and default gateway.
        ...
        **Args**:
            ...
            ``ip``, ``subnetmask``, ``defaultgateway``.
                ...
                **mac**: A string value that represents MAC address of the\
                ...
                ---------------------------------------------------------------
                **ip**: A string value that represents the new IP address in
                dot-decimal notation.\n
                ---------------------------------------------------------------
                **subnetmask**: A string value that represents the new
                subnet mask in dot-decimal notation.\n
                ---------------------------------------------------------------
                **defaultgateway**: A string value that represents the new
    ...
    def __force_ip(self, device_info):
        ...
        self._xsystem.xSystemForceIpAddress(
            mac_int, ip_int, subnetmask_int, defaultgateway_int)

    # add_unicast_discovery_device --------------------------------------------

    def add_unicast_discovery_device(self, ip):
        """
        Registers an IP address for a device on a different subnet than the
        host. Registered devices will be enumerated using unicast discovery
        messages. The list of remote devices will persist until they are
        removed using RemoveUnicastDiscoveryDevice() or until the application
        terminates. Unicast discovery's will be sent when UpdateDevices() is
        called.

        **Args**:
            **ip**: a string value that represents the IP address in dot-\n
            decimal notation.\n
        **------------------------------------------------------------------**\
        **-------------------------------------------------------------------**
        """
        self._xsystem.xSystemAddUnicastDiscoveryDevice(ip)

    # ---------------------------------------------------------------------

 # add_unicast_discovery_device --------------------------------------------

    def remove_unicast_discovery_device(self, ip):
        """
       Unregisters an IP address for a device on a different subnet than the
       host. To remove all registered devices, pass None for the IP address
       argument.

        **Args**:
            **ip**: A string value that represents the IP address in dot-\
            decimal notation, or None.\n
        **------------------------------------------------------------------**\
        **-------------------------------------------------------------------**
        """
        self._xsystem.xSystemRemoveUnicastDiscoveryDevice(ip)
    # ---------------------------------------------------------------------

    def __get_tl_system_nodemap(self):
        hxnodemap = self._xsystem.xSystemGetTLSystemNodeMap()
        nodemap = _Nodemap(hxnodemap)

        nodemap._nodemap_type_name = 'system_tl_system_nodemap'
        return nodemap
    ...
    :type: ``arena_api.nodemap.Nodemap`` instance.\n
    ...
    ``arena_api`` package is imported and deinitialized when the
    package is unloaded. All available nodes can be viewed in
