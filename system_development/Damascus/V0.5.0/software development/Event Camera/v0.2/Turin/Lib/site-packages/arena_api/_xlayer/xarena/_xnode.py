...
class _xRegister():
    ...
    def xRegisterSet(self, data, data_len):
        ...
        harenac.acRegisterSet(
            self.hxnode, 
            np_data.ctypes.data_as(POINTER(uint8_t)), int64_t(data_len))

    def xRegisterGet(self, register_len):

        if register_len < 1:
            register_len = 1
        data = np.zeros((register_len), dtype=np.uint8)
        ...
        harenac.acRegisterGet(
            self.hxnode, 
            data.ctypes.data_as(POINTER(uint8_t)), 
            register_len)
